 ...
 * best to intercept the set slot packet and inject the lore manually (requires PacketEvents),
 *
	...
	public static ItemStack removeEnchantmentLores(final ItemStack item) {
		...
		if (meta != null && meta.hasLore()) {
				...
				for (final Map.Entry<Enchantment, Integer> entry : enchants.entrySet()) {
					...
					if (simpleEnchantment != null) {
						...
						if (loreLine != null && !loreLine.isEmpty())
							colorLess.add(SimpleComponent.fromMiniAmpersand(loreLine).toPlain(null));
					}
				}
			...
			for (final String line : lore)
				if (colorLess.contains(SimpleComponent.fromMiniAmpersand(line).toPlain(null)))
					foEnchanted = true;
				else
					newLore.add(line);
